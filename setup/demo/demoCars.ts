...
async function main() {
    ...
    const responsible = await prisma.responsibles.create({
        data: {
            identifier: 'Cecilia123',
            full_name: 'Cecilia Coelho Silva',
            email: 'cecilia@acompanhar.com',
    ...
    const professional = await prisma.professionals.create({
        data: {
            identifier: 'Caio123',
            full_name: 'Caio Cipriano Santos',
            email: 'caio@acompanhar.com',
            password: await encryptPassword('senhaSegura123'),
    ...
    const names = [
         "Renata", "Ricardo", "Roberta", "Rodrigo", "Rita",
        "Raquel", "Rogério", "Rosa", "Ronaldo", "Raul", "Rebeca",
        "Raimundo", "Rômulo", "Rafaela", "Regina", "Rodolfo", "Rubens",
        "Rayan", "Rosana", "Rafaelina", "Rinaldo", "Romilda", "Rute",
        "Roseli", "Renan", "Rogéria", "Rúbia", "Ravena", "Rosário", "Rafael"
    ];
    ...
    for (let i = 1; i <= 15; i++) {//alterar o 3 para mudar a quantidade de usuários criados. OBS: Max de 30 usuários
        const user = await prisma.clients.create({
            data: {
                ...
                full_name: `${names[i]} Rangel Silva`,//${i} 
    ...
    const atecScale = await prisma.scales.findFirst({
        where: { name: 'CARS' }, // Alterar aqui para mudar a escala
        include: {
            questions: {
                include: {
                    itens: {
    ...
    if (atecScale) {
        ...
        console.log('Escala "CARS" não encontrada.');
    }
